...
public class Billboard extends Mesh3D {
	...
	public Billboard( com.watabou.gltextures.SmartTexture texture ) {
		super( texture, UNIT_QUAD.clone(), QUAD_INDICES.clone() );
	}
	...
	public void uv( RectF frame ) {
		verticesBuffer.position( 3 );  verticesBuffer.put( frame.left );  verticesBuffer.put( frame.top );
		verticesBuffer.position( 8 );  verticesBuffer.put( frame.right ); verticesBuffer.put( frame.top );
		verticesBuffer.position( 13 ); verticesBuffer.put( frame.right ); verticesBuffer.put( frame.bottom );
		verticesBuffer.position( 18 ); verticesBuffer.put( frame.left );  verticesBuffer.put( frame.bottom );
	...
	protected void updateMatrix() {
		...
		Matrix.scale3( matrix, sizeX, sizeY, 1f );
	}
}
